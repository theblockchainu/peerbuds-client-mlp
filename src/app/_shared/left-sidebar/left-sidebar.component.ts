...
export class LeftSidebarComponent implements OnInit {
    ...
    updateSideMenu(collection) {
        ...
        if (collection.topics && collection.topics.length >= 3) {
            this.sidebarMenuItems[0].submenu[0].complete = true;
            completedSections++;
        }
        if (collection.language && collection.language.length > 0) {
            this.sidebarMenuItems[0].submenu[1].complete = true;
            completedSections++;
        }
        if (collection.aboutHost && collection.aboutHost.length > 0) {
            this.sidebarMenuItems[0].submenu[2].complete = true;
            completedSections++;
        }
        if (collection.title && collection.title.length > 0 && collection.headline && collection.headline.length > 0) {
        ...
        if (collection.difficultyLevel && collection.difficultyLevel.length > 0 && collection.notes && collection.notes.length > 0) {
            this.sidebarMenuItems[1].submenu[2].complete = true;
            completedSections++;
        }
        if (collection.maxSpots && collection.maxSpots.length > 0) {
            this.sidebarMenuItems[1].submenu[3].complete = true;
            completedSections++;
        }
        if (collection.imageUrls && collection.imageUrls.length > 0 && collection.videoUrls && collection.videoUrls.length > 0) {
            this.sidebarMenuItems[1].submenu[4].complete = true;
            completedSections++;
        }
        if (collection.price && collection.price.length > 0 && collection.currency && collection.currency.length > 0 && collection.cancellationPolicy && collection.cancellationPolicy.length > 0) {
