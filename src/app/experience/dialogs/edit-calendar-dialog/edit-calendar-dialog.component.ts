...
import { CustomDateFormatter } from '../../experience-page/custom-date-formatter.provider';
import { SelectDateDialogComponent } from '../../experience-page/select-date-dialog/select-date-dialog.component';
...
export class EditCalendarDialogComponent implements OnInit {
    ...
    public ngOnInit() {
        ...
        this.recurring = this._fb.group({
            repeatExperienceGroupOption: ['', Validators.required],
            days: [2],
            weekdays: [1],
            daysRepeat: [2],
            monthsRepeat: [6],
        ...
        if (this.endDay) {
            ...
            this.computedEventCalendar = _.remove(temEventCalendar, entry => {
                return ((moment(this.endDate) < moment(entry.startDateTime) && moment(entry.endDateTime) < this.endDay)
                    ...
                    && entry.collectionId !== this.collection.id //Hack to handle the existing Calendar in eventCalendar when Cohort is deleted
    ...
    public createCalendars(end, days, weekday) {
        let start = end;
        let tempEnd;
        if (this.recurring.value.repeatExperienceGroupOption === 'immediate') {
        ...
        else if (this.recurring.value.repeatExperienceGroupOption === 'days') {
        ...
        else if (this.recurring.value.repeatExperienceGroupOption === 'weekdays') {
