...
import {UcFirstPipe, UcWordsPipe} from 'ngx-pipes/esm';
...
export class ConsoleDashboardComponent implements OnInit {
  ...
  private fetchProfileData() {
      this._profileService.getProfileData(this.userId,  {include: [{'peer': ['reviewsAboutYou', 'collections', 'topicsLearning', 'topicsTeaching']}]}).subscribe((profiles) => {
  ...
  private fetchNotifications() {
    ...
    private fetchOwnedCollections() {
        this._collectionService.getOwnedCollections(this.userId, '{"include": ["calendars", {"owners": "reviewsAboutYou"}, {"participants": ["reviewsAboutYou", "ownedCollections", "profiles"]}, "topics", "views", {"contents":"schedules"}] }', (err, result) => {
            if (err) {
                console.log(err);
            } else {
                this.drafts = [];
                        ...
                        if (now.diff(endDateMoment) < 0) {
                            if (!now.isBetween(startDateMoment, endDateMoment)) {
                                ...
                                if (this.upcomingCollectionsObject[collection.id]['collection'].owners[0].reviewsAboutYou) {
                                    this.upcomingCollectionsObject[collection.id]['collection'].owners[0].reviewsAboutYou.forEach(review => {
                                ...
                                this.upcomingCollectionsObject[collection.id]['collection'].reviewValue = this._collectionService.calculateRating(reviews);
                            } else {
                                ...
                                this.liveCollectionsObject[collection.id]['collection'].reviewValue = this._collectionService.calculateRating(reviews);
                            }
                            ...
                            if (collection.id in this.pastCollectionsObject) {
                                ...
                                this.pastCollectionsObject[collection.id]['collection'].reviewValue = this._collectionService.calculateRating(reviews);
                            }
                        }
                    }
                });
